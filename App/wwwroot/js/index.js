...
// Request APIs
const APIList = {
    GET: [{ "SELECT-ROLE": "role" }, { "SELECT-USER": "user" }, { "SELECT-USER-ROLE-NULL": "user/null" }],
    POST: { "INSERT-ROLE": "role?" },
    PUT: [{ "UPDATE-USER": "user?" }, { "UPDATE-ROLE": "role?" }],
    DELETE: [{ "DELETE-USER": "user?" }, { "DELETE-ROLE": "role?" }, { "DELETE-LOCAL-USER": "local/user?" }],
};
...
let RequestDeleteLocalUser = () => {
    let RequestApiURL = "";
    let APIArgument = "RemoveUserCd=" + $("#RemoveLocalUserCd").val();

    RequestApiURL +=
        defaultRequesteURL + APIList["DELETE"][2]["DELETE-LOCAL-USER"] + APIArgument;
